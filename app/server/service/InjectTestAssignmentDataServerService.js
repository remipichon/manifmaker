...
/** @class InjectTestAssignmentDataServerService */
export class InjectTestAssignmentDataServerService {
  ...
  injectAllData() {
    if (this.init) {
      InjectDataInfo.insert({triggerEnv: "TEST_ASSIGNMENT", date: new Date(), options: this.options});
    }
  ...
  populateAssignmentTerms() {
    ...
    AssignmentTerms.insert({
      name: "Two days 2h strict mode" + this.options.suffix,
    ...
    AssignmentTerms.insert({
      name: "Two days 2h non strict mode" + this.options.suffix,
  ...
  populateTasks() {
    ...
    //tasks
    this.tasks = {};
    this.tasks.pillingUp = Tasks.insert({
      name: "Pilling up sand " + this.options.suffix,
      teamId: this.teams.dreamTeam,
    ...
    this.tasks.pillingUpNonStricte = Tasks.insert({
      ...
      timeSlots: [
        ...
        {
          start: InjectDataHelperServerService.getDateFromDateAndHourMinute(this.options.year, this.options.month, this.options.date + 1 , 12, 0),
    ...
    Tasks.update(this.tasks.pillingUpNonStricte, {$set: {"equipmentValidation.currentState": ValidationState.READY}})
  }
