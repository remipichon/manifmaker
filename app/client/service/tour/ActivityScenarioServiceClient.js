...
export class ActivityScenarioServiceClient {
  ...
  static fillCreateActivityForm(speed, options) {
    return GuidedTourServiceClient.alert("<p>Voici le formulaire de création où nous renseignons uniquement les informations principales.</p>" +
      ...
      .then(() => GuidedTourServiceClient.standardSleep(speed))
      .then(() => GuidedTourServiceClient.selectOption("Place", "Bocal", speed ))
      .then(() => GuidedTourServiceClient.standardSleep(speed))
  ...
  static fillUpdateActivityForm(speed, options) {
    return GuidedTourServiceClient.selectDate(".date-time-picker-update-activity-start input", "10/02/" + options.year, "Sep", "10", speed)
      .then(() => GuidedTourServiceClient.standardSleep(speed))
      .then(() => GuidedTourServiceClient.selectDate(".date-time-picker-update-activity-end input", "10/02/" + options.year, "Sep", "18", speed))
      ...
      .then(() => GuidedTourServiceClient.standardSleep(speed))
      .then(() => GuidedTourServiceClient.selectOption("Water supply", "AIP", speed ))
      .then(() => GuidedTourServiceClient.sleep(500 * speed))
