...
export class EquipmentComponent extends BlazeComponent{
    ...
    equipmentsCategories() {
        ...
        if(this.isEquipmentsReadOnly()){
            ...
            _.each(categories, _.bind(function (category) {
                var equipmentsForACategory = Equipments.find({EquipmentCategories_Id: category._id, targetUsage: {$in: [EquipementTargetUsage.BOTH, this.equipementTargetUsage]}})
                    .fetch();
            ...
            var result = [];
            _.each(categories, _.bind(function (category) {
                if (Equipments.find({EquipmentCategories_Id: category._id, targetUsage: {$in: [EquipementTargetUsage.BOTH,this.equipementTargetUsage]}})
                        .fetch().length !== 0) {
                    result.push(category);
                }
            },this));
    ...
    equipments(category) {
        var equipmentCategory = Equipments.find({EquipmentCategories_Id: category._id, targetUsage: {$in: [EquipementTargetUsage.BOTH, this.equipementTargetUsage]}});
        if(this.isEquipmentsReadOnly()){
            var result;
